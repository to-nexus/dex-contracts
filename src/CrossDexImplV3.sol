...
import {IMarketV3} from "./interfaces/IMarket.sol";
import {IRouter, IRouterInitializer} from "./interfaces/IRouter.sol";

import {WETH} from "./WETH.sol";

contract CrossDexImplV3 is ICrossDex, UUPSUpgradeable, OwnableUpgradeable {
        ...
        returns (address)
    {
        bytes memory bytecode = abi.encodePacked(
            ...
            abi.encode(
                marketImpl, abi.encodeCall(IMarketV3.initialize, (_owner, ROUTER, quote, pairImpl, feeCollector, data))
            )
        );
