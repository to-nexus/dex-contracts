// Code generated via abigen V2 - DO NOT EDIT.
// This file is a generated binding and any manual changes will be lost.

package binding

import (
	"bytes"
	"errors"
	"math/big"

	"github.com/ethereum/go-ethereum/accounts/abi"
	"github.com/ethereum/go-ethereum/accounts/abi/bind/v2"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/core/types"
)

// Reference imports to suppress errors if they are not otherwise used.
var (
	_ = bytes.Equal
	_ = errors.New
	_ = big.NewInt
	_ = common.Big1
	_ = types.BloomLookup
	_ = abi.ConvertType
)

// IPairConfig is an auto generated low-level Go binding around an user-defined struct.

// IPairOrder is an auto generated low-level Go binding around an user-defined struct.

// PairImplMetaData contains all meta data concerning the PairImpl contract.
var PairImplMetaData = bind.MetaData{
	ABI: "[{\"inputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"constructor\"},{\"inputs\":[],\"name\":\"BASE\",\"outputs\":[{\"internalType\":\"contractIERC20\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"DENOMINATOR\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"MARKET\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"QUOTE\",\"outputs\":[{\"internalType\":\"contractIERC20\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"ROUTER\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"UPGRADE_INTERFACE_VERSION\",\"outputs\":[{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"account\",\"type\":\"address\"}],\"name\":\"accountReserves\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"base\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"quote\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"baseReserve\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"caller\",\"type\":\"address\"},{\"internalType\":\"uint256[]\",\"name\":\"orderIds\",\"type\":\"uint256[]\"}],\"name\":\"cancelOrder\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256[]\",\"name\":\"orderIds\",\"type\":\"uint256[]\"}],\"name\":\"emergencyCancelOrder\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"side\",\"type\":\"uint8\"},{\"internalType\":\"uint256\",\"name\":\"price\",\"type\":\"uint256\"},{\"internalType\":\"uint256[2]\",\"name\":\"adjacent\",\"type\":\"uint256[2]\"},{\"internalType\":\"uint256\",\"name\":\"findMaxCount\",\"type\":\"uint256\"}],\"name\":\"findPrevPrice\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"getConfig\",\"outputs\":[{\"components\":[{\"internalType\":\"contractIERC20\",\"name\":\"QUOTE\",\"type\":\"address\"},{\"internalType\":\"contractIERC20\",\"name\":\"BASE\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"DENOMINATOR\",\"type\":\"uint256\"}],\"internalType\":\"structIPair.Config\",\"name\":\"\",\"type\":\"tuple\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"router\",\"type\":\"address\"},{\"internalType\":\"address\",\"name\":\"quote\",\"type\":\"address\"},{\"internalType\":\"address\",\"name\":\"base\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"_tickSize\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"_lotSize\",\"type\":\"uint256\"}],\"name\":\"initialize\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"lotSize\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"matchedAt\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"matchedPrice\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"minTradeVolume\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"id\",\"type\":\"uint256\"}],\"name\":\"orderById\",\"outputs\":[{\"components\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"side\",\"type\":\"uint8\"},{\"internalType\":\"address\",\"name\":\"owner\",\"type\":\"address\"},{\"internalType\":\"uint32\",\"name\":\"feeBps\",\"type\":\"uint32\"},{\"internalType\":\"uint256\",\"name\":\"price\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"}],\"internalType\":\"structIPair.Order\",\"name\":\"\",\"type\":\"tuple\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"side\",\"type\":\"uint8\"},{\"internalType\":\"uint256[]\",\"name\":\"prices\",\"type\":\"uint256[]\"}],\"name\":\"ordersByPrices\",\"outputs\":[{\"internalType\":\"uint256[][]\",\"name\":\"\",\"type\":\"uint256[][]\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"owner\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"paused\",\"outputs\":[{\"internalType\":\"bool\",\"name\":\"\",\"type\":\"bool\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"proxiableUUID\",\"outputs\":[{\"internalType\":\"bytes32\",\"name\":\"\",\"type\":\"bytes32\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"quoteReserve\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"bool\",\"name\":\"pause\",\"type\":\"bool\"}],\"name\":\"setPause\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"_lotSize\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"_tickSize\",\"type\":\"uint256\"}],\"name\":\"setTickSize\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"contractIERC20\",\"name\":\"erc20\",\"type\":\"address\"},{\"internalType\":\"address\",\"name\":\"to\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"}],\"name\":\"skim\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"components\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"side\",\"type\":\"uint8\"},{\"internalType\":\"address\",\"name\":\"owner\",\"type\":\"address\"},{\"internalType\":\"uint32\",\"name\":\"feeBps\",\"type\":\"uint32\"},{\"internalType\":\"uint256\",\"name\":\"price\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"}],\"internalType\":\"structIPair.Order\",\"name\":\"order\",\"type\":\"tuple\"},{\"internalType\":\"enumIPair.LimitConstraints\",\"name\":\"constraints\",\"type\":\"uint8\"},{\"internalType\":\"uint256\",\"name\":\"prevPrice\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"maxMatchCount\",\"type\":\"uint256\"}],\"name\":\"submitLimitOrder\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"orderId\",\"type\":\"uint256\"}],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"components\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"side\",\"type\":\"uint8\"},{\"internalType\":\"address\",\"name\":\"owner\",\"type\":\"address\"},{\"internalType\":\"uint32\",\"name\":\"feeBps\",\"type\":\"uint32\"},{\"internalType\":\"uint256\",\"name\":\"price\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"}],\"internalType\":\"structIPair.Order\",\"name\":\"order\",\"type\":\"tuple\"},{\"internalType\":\"uint256\",\"name\":\"spendAmount\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"maxMatchCount\",\"type\":\"uint256\"}],\"name\":\"submitMarketOrder\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"tickSize\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"tickSizes\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"tick\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"lot\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"ticks\",\"outputs\":[{\"internalType\":\"uint256[]\",\"name\":\"sellPrices\",\"type\":\"uint256[]\"},{\"internalType\":\"uint256[]\",\"name\":\"buyPrices\",\"type\":\"uint256[]\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"newImplementation\",\"type\":\"address\"},{\"internalType\":\"bytes\",\"name\":\"data\",\"type\":\"bytes\"}],\"name\":\"upgradeToAndCall\",\"outputs\":[],\"stateMutability\":\"payable\",\"type\":\"function\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"uint256\",\"name\":\"orderId\",\"type\":\"uint256\"},{\"indexed\":true,\"internalType\":\"address\",\"name\":\"owner\",\"type\":\"address\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"},{\"indexed\":true,\"internalType\":\"address\",\"name\":\"recipient\",\"type\":\"address\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"feeBps\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"fee\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"value\",\"type\":\"uint256\"}],\"name\":\"FeeCollect\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"uint64\",\"name\":\"version\",\"type\":\"uint64\"}],\"name\":\"Initialized\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"uint256\",\"name\":\"orderId\",\"type\":\"uint256\"},{\"indexed\":true,\"internalType\":\"enumIPair.CloseType\",\"name\":\"closeType\",\"type\":\"uint8\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"timestamp\",\"type\":\"uint256\"}],\"name\":\"OrderClosed\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"address\",\"name\":\"owner\",\"type\":\"address\"},{\"indexed\":true,\"internalType\":\"uint256\",\"name\":\"orderId\",\"type\":\"uint256\"},{\"indexed\":true,\"internalType\":\"enumIPair.OrderSide\",\"name\":\"side\",\"type\":\"uint8\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"price\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"timestamp\",\"type\":\"uint256\"}],\"name\":\"OrderCreated\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"uint256\",\"name\":\"sellId\",\"type\":\"uint256\"},{\"indexed\":true,\"internalType\":\"uint256\",\"name\":\"buyId\",\"type\":\"uint256\"},{\"indexed\":true,\"internalType\":\"uint256\",\"name\":\"price\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"timestamp\",\"type\":\"uint256\"}],\"name\":\"OrderMatched\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"address\",\"name\":\"account\",\"type\":\"address\"}],\"name\":\"Paused\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"address\",\"name\":\"caller\",\"type\":\"address\"},{\"indexed\":true,\"internalType\":\"address\",\"name\":\"erc20\",\"type\":\"address\"},{\"indexed\":true,\"internalType\":\"address\",\"name\":\"to\",\"type\":\"address\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"amount\",\"type\":\"uint256\"}],\"name\":\"Skim\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"beforeLotSize\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"newLotSize\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"beforeTickSize\",\"type\":\"uint256\"},{\"indexed\":false,\"internalType\":\"uint256\",\"name\":\"newTickSize\",\"type\":\"uint256\"}],\"name\":\"TickSizeUpdated\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"address\",\"name\":\"account\",\"type\":\"address\"}],\"name\":\"Unpaused\",\"type\":\"event\"},{\"anonymous\":false,\"inputs\":[{\"indexed\":true,\"internalType\":\"address\",\"name\":\"implementation\",\"type\":\"address\"}],\"name\":\"Upgraded\",\"type\":\"event\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"target\",\"type\":\"address\"}],\"name\":\"AddressEmptyCode\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"implementation\",\"type\":\"address\"}],\"name\":\"ERC1967InvalidImplementation\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ERC1967NonPayable\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"EnforcedPause\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ExpectedPause\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"FailedCall\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"InvalidInitialization\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ListFailToFindPrev\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ListInvalidFindMaxCount\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ListInvalidIndex\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ListInvalidPrevNode\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"ListZeroData\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"NotInitializing\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"account\",\"type\":\"address\"}],\"name\":\"OwnableUnauthorizedAccount\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"name\":\"PairFillOrKill\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"PairInsufficientTradeVolume\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"},{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"name\":\"PairInvalidAccountReserve\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"PairInvalidAmount\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"bytes32\",\"name\":\"\",\"type\":\"bytes32\"}],\"name\":\"PairInvalidInitializeData\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"PairInvalidOrderId\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"\",\"type\":\"uint8\"}],\"name\":\"PairInvalidOrderSide\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"\",\"type\":\"uint8\"},{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"PairInvalidPrevPrice\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"PairInvalidPrice\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"name\":\"PairInvalidReserve\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"name\":\"PairInvalidRouter\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"PairInvalidTickSize\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"},{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"name\":\"PairNotOwner\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"enumIPair.OrderSide\",\"name\":\"\",\"type\":\"uint8\"},{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"PairUnknownPrices\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"token\",\"type\":\"address\"}],\"name\":\"SafeERC20FailedOperation\",\"type\":\"error\"},{\"inputs\":[],\"name\":\"UUPSUnauthorizedCallContext\",\"type\":\"error\"},{\"inputs\":[{\"internalType\":\"bytes32\",\"name\":\"slot\",\"type\":\"bytes32\"}],\"name\":\"UUPSUnsupportedProxiableUUID\",\"type\":\"error\"}]",
	ID:  "PairImpl",
	Bin: "",
}

// PairImpl is an auto generated Go binding around an Ethereum contract.
type PairImpl struct {
	abi abi.ABI
}

// NewPairImpl creates a new instance of PairImpl.
func NewPairImpl() *PairImpl {
	parsed, err := PairImplMetaData.ParseABI()
	if err != nil {
		panic(errors.New("invalid ABI: " + err.Error()))
	}
	return &PairImpl{abi: *parsed}
}

// Instance creates a wrapper for a deployed contract instance at the given address.
// Use this to create the instance object passed to abigen v2 library functions Call, Transact, etc.
func (c *PairImpl) Instance(backend bind.ContractBackend, addr common.Address) *bind.BoundContract {
	return bind.NewBoundContract(addr, c.abi, backend, backend, backend)
}

// PackBASE is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xec342ad0.
//
// Solidity: function BASE() view returns(address)
func (pairImpl *PairImpl) PackBASE() []byte {
	enc, err := pairImpl.abi.Pack("BASE")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackBASE is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xec342ad0.
//
// Solidity: function BASE() view returns(address)
func (pairImpl *PairImpl) UnpackBASE(data []byte) (common.Address, error) {
	out, err := pairImpl.abi.Unpack("BASE", data)
	if err != nil {
		return *new(common.Address), err
	}
	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	return out0, err
}

// PackDENOMINATOR is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x918f8674.
//
// Solidity: function DENOMINATOR() view returns(uint256)
func (pairImpl *PairImpl) PackDENOMINATOR() []byte {
	enc, err := pairImpl.abi.Pack("DENOMINATOR")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackDENOMINATOR is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x918f8674.
//
// Solidity: function DENOMINATOR() view returns(uint256)
func (pairImpl *PairImpl) UnpackDENOMINATOR(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("DENOMINATOR", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackMARKET is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xf46f16c2.
//
// Solidity: function MARKET() view returns(address)
func (pairImpl *PairImpl) PackMARKET() []byte {
	enc, err := pairImpl.abi.Pack("MARKET")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackMARKET is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xf46f16c2.
//
// Solidity: function MARKET() view returns(address)
func (pairImpl *PairImpl) UnpackMARKET(data []byte) (common.Address, error) {
	out, err := pairImpl.abi.Unpack("MARKET", data)
	if err != nil {
		return *new(common.Address), err
	}
	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	return out0, err
}

// PackQUOTE is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x9c579839.
//
// Solidity: function QUOTE() view returns(address)
func (pairImpl *PairImpl) PackQUOTE() []byte {
	enc, err := pairImpl.abi.Pack("QUOTE")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackQUOTE is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x9c579839.
//
// Solidity: function QUOTE() view returns(address)
func (pairImpl *PairImpl) UnpackQUOTE(data []byte) (common.Address, error) {
	out, err := pairImpl.abi.Unpack("QUOTE", data)
	if err != nil {
		return *new(common.Address), err
	}
	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	return out0, err
}

// PackROUTER is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x32fe7b26.
//
// Solidity: function ROUTER() view returns(address)
func (pairImpl *PairImpl) PackROUTER() []byte {
	enc, err := pairImpl.abi.Pack("ROUTER")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackROUTER is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x32fe7b26.
//
// Solidity: function ROUTER() view returns(address)
func (pairImpl *PairImpl) UnpackROUTER(data []byte) (common.Address, error) {
	out, err := pairImpl.abi.Unpack("ROUTER", data)
	if err != nil {
		return *new(common.Address), err
	}
	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	return out0, err
}

// PackUPGRADEINTERFACEVERSION is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xad3cb1cc.
//
// Solidity: function UPGRADE_INTERFACE_VERSION() view returns(string)
func (pairImpl *PairImpl) PackUPGRADEINTERFACEVERSION() []byte {
	enc, err := pairImpl.abi.Pack("UPGRADE_INTERFACE_VERSION")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackUPGRADEINTERFACEVERSION is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xad3cb1cc.
//
// Solidity: function UPGRADE_INTERFACE_VERSION() view returns(string)
func (pairImpl *PairImpl) UnpackUPGRADEINTERFACEVERSION(data []byte) (string, error) {
	out, err := pairImpl.abi.Unpack("UPGRADE_INTERFACE_VERSION", data)
	if err != nil {
		return *new(string), err
	}
	out0 := *abi.ConvertType(out[0], new(string)).(*string)
	return out0, err
}

// PackAccountReserves is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x7aae3523.
//
// Solidity: function accountReserves(address account) view returns(uint256 base, uint256 quote)
func (pairImpl *PairImpl) PackAccountReserves(account common.Address) []byte {
	enc, err := pairImpl.abi.Pack("accountReserves", account)
	if err != nil {
		panic(err)
	}
	return enc
}

// AccountReservesOutput serves as a container for the return parameters of contract
// method AccountReserves.

// UnpackAccountReserves is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x7aae3523.
//
// Solidity: function accountReserves(address account) view returns(uint256 base, uint256 quote)
func (pairImpl *PairImpl) UnpackAccountReserves(data []byte) (AccountReservesOutput, error) {
	out, err := pairImpl.abi.Unpack("accountReserves", data)
	outstruct := new(AccountReservesOutput)
	if err != nil {
		return *outstruct, err
	}
	outstruct.Base = abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	outstruct.Quote = abi.ConvertType(out[1], new(big.Int)).(*big.Int)
	return *outstruct, err

}

// PackBaseReserve is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xdfdf2a72.
//
// Solidity: function baseReserve() view returns(uint256)
func (pairImpl *PairImpl) PackBaseReserve() []byte {
	enc, err := pairImpl.abi.Pack("baseReserve")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackBaseReserve is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xdfdf2a72.
//
// Solidity: function baseReserve() view returns(uint256)
func (pairImpl *PairImpl) UnpackBaseReserve(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("baseReserve", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackCancelOrder is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x1ec482d7.
//
// Solidity: function cancelOrder(address caller, uint256[] orderIds) returns()
func (pairImpl *PairImpl) PackCancelOrder(caller common.Address, orderIds []*big.Int) []byte {
	enc, err := pairImpl.abi.Pack("cancelOrder", caller, orderIds)
	if err != nil {
		panic(err)
	}
	return enc
}

// PackEmergencyCancelOrder is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xe077dd8a.
//
// Solidity: function emergencyCancelOrder(uint256[] orderIds) returns()
func (pairImpl *PairImpl) PackEmergencyCancelOrder(orderIds []*big.Int) []byte {
	enc, err := pairImpl.abi.Pack("emergencyCancelOrder", orderIds)
	if err != nil {
		panic(err)
	}
	return enc
}

// PackFindPrevPrice is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x6c4cfbc8.
//
// Solidity: function findPrevPrice(uint8 side, uint256 price, uint256[2] adjacent, uint256 findMaxCount) view returns(uint256)
func (pairImpl *PairImpl) PackFindPrevPrice(side uint8, price *big.Int, adjacent [2]*big.Int, findMaxCount *big.Int) []byte {
	enc, err := pairImpl.abi.Pack("findPrevPrice", side, price, adjacent, findMaxCount)
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackFindPrevPrice is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x6c4cfbc8.
//
// Solidity: function findPrevPrice(uint8 side, uint256 price, uint256[2] adjacent, uint256 findMaxCount) view returns(uint256)
func (pairImpl *PairImpl) UnpackFindPrevPrice(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("findPrevPrice", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackGetConfig is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xc3f909d4.
//
// Solidity: function getConfig() view returns((address,address,uint256))
func (pairImpl *PairImpl) PackGetConfig() []byte {
	enc, err := pairImpl.abi.Pack("getConfig")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackGetConfig is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xc3f909d4.
//
// Solidity: function getConfig() view returns((address,address,uint256))
func (pairImpl *PairImpl) UnpackGetConfig(data []byte) (IPairConfig, error) {
	out, err := pairImpl.abi.Unpack("getConfig", data)
	if err != nil {
		return *new(IPairConfig), err
	}
	out0 := *abi.ConvertType(out[0], new(IPairConfig)).(*IPairConfig)
	return out0, err
}

// PackInitialize is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xa6b63eb8.
//
// Solidity: function initialize(address router, address quote, address base, uint256 _tickSize, uint256 _lotSize) returns()
func (pairImpl *PairImpl) PackInitialize(router common.Address, quote common.Address, base common.Address, tickSize *big.Int, lotSize *big.Int) []byte {
	enc, err := pairImpl.abi.Pack("initialize", router, quote, base, tickSize, lotSize)
	if err != nil {
		panic(err)
	}
	return enc
}

// PackLotSize is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x4942f65f.
//
// Solidity: function lotSize() view returns(uint256)
func (pairImpl *PairImpl) PackLotSize() []byte {
	enc, err := pairImpl.abi.Pack("lotSize")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackLotSize is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x4942f65f.
//
// Solidity: function lotSize() view returns(uint256)
func (pairImpl *PairImpl) UnpackLotSize(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("lotSize", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackMatchedAt is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x4d6754f1.
//
// Solidity: function matchedAt() view returns(uint256)
func (pairImpl *PairImpl) PackMatchedAt() []byte {
	enc, err := pairImpl.abi.Pack("matchedAt")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackMatchedAt is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x4d6754f1.
//
// Solidity: function matchedAt() view returns(uint256)
func (pairImpl *PairImpl) UnpackMatchedAt(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("matchedAt", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackMatchedPrice is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x1edd8614.
//
// Solidity: function matchedPrice() view returns(uint256)
func (pairImpl *PairImpl) PackMatchedPrice() []byte {
	enc, err := pairImpl.abi.Pack("matchedPrice")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackMatchedPrice is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x1edd8614.
//
// Solidity: function matchedPrice() view returns(uint256)
func (pairImpl *PairImpl) UnpackMatchedPrice(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("matchedPrice", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackMinTradeVolume is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x7cbf6db2.
//
// Solidity: function minTradeVolume() view returns(uint256)
func (pairImpl *PairImpl) PackMinTradeVolume() []byte {
	enc, err := pairImpl.abi.Pack("minTradeVolume")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackMinTradeVolume is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x7cbf6db2.
//
// Solidity: function minTradeVolume() view returns(uint256)
func (pairImpl *PairImpl) UnpackMinTradeVolume(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("minTradeVolume", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackOrderById is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x8d77eba5.
//
// Solidity: function orderById(uint256 id) view returns((uint8,address,uint32,uint256,uint256))
func (pairImpl *PairImpl) PackOrderById(id *big.Int) []byte {
	enc, err := pairImpl.abi.Pack("orderById", id)
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackOrderById is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x8d77eba5.
//
// Solidity: function orderById(uint256 id) view returns((uint8,address,uint32,uint256,uint256))
func (pairImpl *PairImpl) UnpackOrderById(data []byte) (IPairOrder, error) {
	out, err := pairImpl.abi.Unpack("orderById", data)
	if err != nil {
		return *new(IPairOrder), err
	}
	out0 := *abi.ConvertType(out[0], new(IPairOrder)).(*IPairOrder)
	return out0, err
}

// PackOrdersByPrices is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x12a808cc.
//
// Solidity: function ordersByPrices(uint8 side, uint256[] prices) view returns(uint256[][])
func (pairImpl *PairImpl) PackOrdersByPrices(side uint8, prices []*big.Int) []byte {
	enc, err := pairImpl.abi.Pack("ordersByPrices", side, prices)
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackOrdersByPrices is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x12a808cc.
//
// Solidity: function ordersByPrices(uint8 side, uint256[] prices) view returns(uint256[][])
func (pairImpl *PairImpl) UnpackOrdersByPrices(data []byte) ([][]*big.Int, error) {
	out, err := pairImpl.abi.Unpack("ordersByPrices", data)
	if err != nil {
		return *new([][]*big.Int), err
	}
	out0 := *abi.ConvertType(out[0], new([][]*big.Int)).(*[][]*big.Int)
	return out0, err
}

// PackOwner is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (pairImpl *PairImpl) PackOwner() []byte {
	enc, err := pairImpl.abi.Pack("owner")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackOwner is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (pairImpl *PairImpl) UnpackOwner(data []byte) (common.Address, error) {
	out, err := pairImpl.abi.Unpack("owner", data)
	if err != nil {
		return *new(common.Address), err
	}
	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	return out0, err
}

// PackPaused is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x5c975abb.
//
// Solidity: function paused() view returns(bool)
func (pairImpl *PairImpl) PackPaused() []byte {
	enc, err := pairImpl.abi.Pack("paused")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackPaused is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x5c975abb.
//
// Solidity: function paused() view returns(bool)
func (pairImpl *PairImpl) UnpackPaused(data []byte) (bool, error) {
	out, err := pairImpl.abi.Unpack("paused", data)
	if err != nil {
		return *new(bool), err
	}
	out0 := *abi.ConvertType(out[0], new(bool)).(*bool)
	return out0, err
}

// PackProxiableUUID is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x52d1902d.
//
// Solidity: function proxiableUUID() view returns(bytes32)
func (pairImpl *PairImpl) PackProxiableUUID() []byte {
	enc, err := pairImpl.abi.Pack("proxiableUUID")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackProxiableUUID is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x52d1902d.
//
// Solidity: function proxiableUUID() view returns(bytes32)
func (pairImpl *PairImpl) UnpackProxiableUUID(data []byte) ([32]byte, error) {
	out, err := pairImpl.abi.Unpack("proxiableUUID", data)
	if err != nil {
		return *new([32]byte), err
	}
	out0 := *abi.ConvertType(out[0], new([32]byte)).(*[32]byte)
	return out0, err
}

// PackQuoteReserve is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x9da771f4.
//
// Solidity: function quoteReserve() view returns(uint256)
func (pairImpl *PairImpl) PackQuoteReserve() []byte {
	enc, err := pairImpl.abi.Pack("quoteReserve")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackQuoteReserve is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x9da771f4.
//
// Solidity: function quoteReserve() view returns(uint256)
func (pairImpl *PairImpl) UnpackQuoteReserve(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("quoteReserve", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackSetPause is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xbedb86fb.
//
// Solidity: function setPause(bool pause) returns()
func (pairImpl *PairImpl) PackSetPause(pause bool) []byte {
	enc, err := pairImpl.abi.Pack("setPause", pause)
	if err != nil {
		panic(err)
	}
	return enc
}

// PackSetTickSize is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xfe566349.
//
// Solidity: function setTickSize(uint256 _lotSize, uint256 _tickSize) returns()
func (pairImpl *PairImpl) PackSetTickSize(lotSize *big.Int, tickSize *big.Int) []byte {
	enc, err := pairImpl.abi.Pack("setTickSize", lotSize, tickSize)
	if err != nil {
		panic(err)
	}
	return enc
}

// PackSkim is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x1a0361fb.
//
// Solidity: function skim(address erc20, address to, uint256 amount) returns()
func (pairImpl *PairImpl) PackSkim(erc20 common.Address, to common.Address, amount *big.Int) []byte {
	enc, err := pairImpl.abi.Pack("skim", erc20, to, amount)
	if err != nil {
		panic(err)
	}
	return enc
}

// PackSubmitLimitOrder is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x2dd6a00a.
//
// Solidity: function submitLimitOrder((uint8,address,uint32,uint256,uint256) order, uint8 constraints, uint256 prevPrice, uint256 maxMatchCount) returns(uint256 orderId)
func (pairImpl *PairImpl) PackSubmitLimitOrder(order IPairOrder, constraints uint8, prevPrice *big.Int, maxMatchCount *big.Int) []byte {
	enc, err := pairImpl.abi.Pack("submitLimitOrder", order, constraints, prevPrice, maxMatchCount)
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackSubmitLimitOrder is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x2dd6a00a.
//
// Solidity: function submitLimitOrder((uint8,address,uint32,uint256,uint256) order, uint8 constraints, uint256 prevPrice, uint256 maxMatchCount) returns(uint256 orderId)
func (pairImpl *PairImpl) UnpackSubmitLimitOrder(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("submitLimitOrder", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackSubmitMarketOrder is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xcc50e36a.
//
// Solidity: function submitMarketOrder((uint8,address,uint32,uint256,uint256) order, uint256 spendAmount, uint256 maxMatchCount) returns()
func (pairImpl *PairImpl) PackSubmitMarketOrder(order IPairOrder, spendAmount *big.Int, maxMatchCount *big.Int) []byte {
	enc, err := pairImpl.abi.Pack("submitMarketOrder", order, spendAmount, maxMatchCount)
	if err != nil {
		panic(err)
	}
	return enc
}

// PackTickSize is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xf210d087.
//
// Solidity: function tickSize() view returns(uint256)
func (pairImpl *PairImpl) PackTickSize() []byte {
	enc, err := pairImpl.abi.Pack("tickSize")
	if err != nil {
		panic(err)
	}
	return enc
}

// UnpackTickSize is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xf210d087.
//
// Solidity: function tickSize() view returns(uint256)
func (pairImpl *PairImpl) UnpackTickSize(data []byte) (*big.Int, error) {
	out, err := pairImpl.abi.Unpack("tickSize", data)
	if err != nil {
		return new(big.Int), err
	}
	out0 := abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	return out0, err
}

// PackTickSizes is the Go binding used to pack the parameters required for calling
// the contract method with ID 0xd6e9cc2a.
//
// Solidity: function tickSizes() view returns(uint256 tick, uint256 lot)
func (pairImpl *PairImpl) PackTickSizes() []byte {
	enc, err := pairImpl.abi.Pack("tickSizes")
	if err != nil {
		panic(err)
	}
	return enc
}

// TickSizesOutput serves as a container for the return parameters of contract
// method TickSizes.

// UnpackTickSizes is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0xd6e9cc2a.
//
// Solidity: function tickSizes() view returns(uint256 tick, uint256 lot)
func (pairImpl *PairImpl) UnpackTickSizes(data []byte) (TickSizesOutput, error) {
	out, err := pairImpl.abi.Unpack("tickSizes", data)
	outstruct := new(TickSizesOutput)
	if err != nil {
		return *outstruct, err
	}
	outstruct.Tick = abi.ConvertType(out[0], new(big.Int)).(*big.Int)
	outstruct.Lot = abi.ConvertType(out[1], new(big.Int)).(*big.Int)
	return *outstruct, err

}

// PackTicks is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x2cfffaf6.
//
// Solidity: function ticks() view returns(uint256[] sellPrices, uint256[] buyPrices)
func (pairImpl *PairImpl) PackTicks() []byte {
	enc, err := pairImpl.abi.Pack("ticks")
	if err != nil {
		panic(err)
	}
	return enc
}

// TicksOutput serves as a container for the return parameters of contract
// method Ticks.

// UnpackTicks is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x2cfffaf6.
//
// Solidity: function ticks() view returns(uint256[] sellPrices, uint256[] buyPrices)
func (pairImpl *PairImpl) UnpackTicks(data []byte) (TicksOutput, error) {
	out, err := pairImpl.abi.Unpack("ticks", data)
	outstruct := new(TicksOutput)
	if err != nil {
		return *outstruct, err
	}
	outstruct.SellPrices = *abi.ConvertType(out[0], new([]*big.Int)).(*[]*big.Int)
	outstruct.BuyPrices = *abi.ConvertType(out[1], new([]*big.Int)).(*[]*big.Int)
	return *outstruct, err

}

// PackUpgradeToAndCall is the Go binding used to pack the parameters required for calling
// the contract method with ID 0x4f1ef286.
//
// Solidity: function upgradeToAndCall(address newImplementation, bytes data) payable returns()
func (pairImpl *PairImpl) PackUpgradeToAndCall(newImplementation common.Address, data []byte) []byte {
	enc, err := pairImpl.abi.Pack("upgradeToAndCall", newImplementation, data)
	if err != nil {
		panic(err)
	}
	return enc
}

// PairImplFeeCollect represents a FeeCollect event raised by the PairImpl contract.
type PairImplFeeCollect struct {
	OrderId   *big.Int
	Owner     common.Address
	Amount    *big.Int
	Recipient common.Address
	FeeBps    *big.Int
	Fee       *big.Int
	Value     *big.Int
	Raw       *types.Log // Blockchain specific contextual infos
}

const PairImplFeeCollectEventName = "FeeCollect"

// ContractEventName returns the user-defined event name.
func (PairImplFeeCollect) ContractEventName() string {
	return PairImplFeeCollectEventName
}

// UnpackFeeCollectEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event FeeCollect(uint256 indexed orderId, address indexed owner, uint256 amount, address indexed recipient, uint256 feeBps, uint256 fee, uint256 value)
func (pairImpl *PairImpl) UnpackFeeCollectEvent(log *types.Log) (*PairImplFeeCollect, error) {
	event := "FeeCollect"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplFeeCollect)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplInitialized represents a Initialized event raised by the PairImpl contract.
type PairImplInitialized struct {
	Version uint64
	Raw     *types.Log // Blockchain specific contextual infos
}

const PairImplInitializedEventName = "Initialized"

// ContractEventName returns the user-defined event name.
func (PairImplInitialized) ContractEventName() string {
	return PairImplInitializedEventName
}

// UnpackInitializedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event Initialized(uint64 version)
func (pairImpl *PairImpl) UnpackInitializedEvent(log *types.Log) (*PairImplInitialized, error) {
	event := "Initialized"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplInitialized)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplOrderClosed represents a OrderClosed event raised by the PairImpl contract.
type PairImplOrderClosed struct {
	OrderId   *big.Int
	CloseType uint8
	Timestamp *big.Int
	Raw       *types.Log // Blockchain specific contextual infos
}

const PairImplOrderClosedEventName = "OrderClosed"

// ContractEventName returns the user-defined event name.
func (PairImplOrderClosed) ContractEventName() string {
	return PairImplOrderClosedEventName
}

// UnpackOrderClosedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event OrderClosed(uint256 indexed orderId, uint8 indexed closeType, uint256 timestamp)
func (pairImpl *PairImpl) UnpackOrderClosedEvent(log *types.Log) (*PairImplOrderClosed, error) {
	event := "OrderClosed"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplOrderClosed)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplOrderCreated represents a OrderCreated event raised by the PairImpl contract.
type PairImplOrderCreated struct {
	Owner     common.Address
	OrderId   *big.Int
	Side      uint8
	Price     *big.Int
	Amount    *big.Int
	Timestamp *big.Int
	Raw       *types.Log // Blockchain specific contextual infos
}

const PairImplOrderCreatedEventName = "OrderCreated"

// ContractEventName returns the user-defined event name.
func (PairImplOrderCreated) ContractEventName() string {
	return PairImplOrderCreatedEventName
}

// UnpackOrderCreatedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event OrderCreated(address indexed owner, uint256 indexed orderId, uint8 indexed side, uint256 price, uint256 amount, uint256 timestamp)
func (pairImpl *PairImpl) UnpackOrderCreatedEvent(log *types.Log) (*PairImplOrderCreated, error) {
	event := "OrderCreated"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplOrderCreated)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplOrderMatched represents a OrderMatched event raised by the PairImpl contract.
type PairImplOrderMatched struct {
	SellId    *big.Int
	BuyId     *big.Int
	Price     *big.Int
	Amount    *big.Int
	Timestamp *big.Int
	Raw       *types.Log // Blockchain specific contextual infos
}

const PairImplOrderMatchedEventName = "OrderMatched"

// ContractEventName returns the user-defined event name.
func (PairImplOrderMatched) ContractEventName() string {
	return PairImplOrderMatchedEventName
}

// UnpackOrderMatchedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event OrderMatched(uint256 indexed sellId, uint256 indexed buyId, uint256 indexed price, uint256 amount, uint256 timestamp)
func (pairImpl *PairImpl) UnpackOrderMatchedEvent(log *types.Log) (*PairImplOrderMatched, error) {
	event := "OrderMatched"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplOrderMatched)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplPaused represents a Paused event raised by the PairImpl contract.
type PairImplPaused struct {
	Account common.Address
	Raw     *types.Log // Blockchain specific contextual infos
}

const PairImplPausedEventName = "Paused"

// ContractEventName returns the user-defined event name.
func (PairImplPaused) ContractEventName() string {
	return PairImplPausedEventName
}

// UnpackPausedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event Paused(address account)
func (pairImpl *PairImpl) UnpackPausedEvent(log *types.Log) (*PairImplPaused, error) {
	event := "Paused"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplPaused)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplSkim represents a Skim event raised by the PairImpl contract.
type PairImplSkim struct {
	Caller common.Address
	Erc20  common.Address
	To     common.Address
	Amount *big.Int
	Raw    *types.Log // Blockchain specific contextual infos
}

const PairImplSkimEventName = "Skim"

// ContractEventName returns the user-defined event name.
func (PairImplSkim) ContractEventName() string {
	return PairImplSkimEventName
}

// UnpackSkimEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event Skim(address indexed caller, address indexed erc20, address indexed to, uint256 amount)
func (pairImpl *PairImpl) UnpackSkimEvent(log *types.Log) (*PairImplSkim, error) {
	event := "Skim"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplSkim)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplTickSizeUpdated represents a TickSizeUpdated event raised by the PairImpl contract.
type PairImplTickSizeUpdated struct {
	BeforeLotSize  *big.Int
	NewLotSize     *big.Int
	BeforeTickSize *big.Int
	NewTickSize    *big.Int
	Raw            *types.Log // Blockchain specific contextual infos
}

const PairImplTickSizeUpdatedEventName = "TickSizeUpdated"

// ContractEventName returns the user-defined event name.
func (PairImplTickSizeUpdated) ContractEventName() string {
	return PairImplTickSizeUpdatedEventName
}

// UnpackTickSizeUpdatedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event TickSizeUpdated(uint256 beforeLotSize, uint256 newLotSize, uint256 beforeTickSize, uint256 newTickSize)
func (pairImpl *PairImpl) UnpackTickSizeUpdatedEvent(log *types.Log) (*PairImplTickSizeUpdated, error) {
	event := "TickSizeUpdated"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplTickSizeUpdated)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplUnpaused represents a Unpaused event raised by the PairImpl contract.
type PairImplUnpaused struct {
	Account common.Address
	Raw     *types.Log // Blockchain specific contextual infos
}

const PairImplUnpausedEventName = "Unpaused"

// ContractEventName returns the user-defined event name.
func (PairImplUnpaused) ContractEventName() string {
	return PairImplUnpausedEventName
}

// UnpackUnpausedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event Unpaused(address account)
func (pairImpl *PairImpl) UnpackUnpausedEvent(log *types.Log) (*PairImplUnpaused, error) {
	event := "Unpaused"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplUnpaused)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// PairImplUpgraded represents a Upgraded event raised by the PairImpl contract.
type PairImplUpgraded struct {
	Implementation common.Address
	Raw            *types.Log // Blockchain specific contextual infos
}

const PairImplUpgradedEventName = "Upgraded"

// ContractEventName returns the user-defined event name.
func (PairImplUpgraded) ContractEventName() string {
	return PairImplUpgradedEventName
}

// UnpackUpgradedEvent is the Go binding that unpacks the event data emitted
// by contract.
//
// Solidity: event Upgraded(address indexed implementation)
func (pairImpl *PairImpl) UnpackUpgradedEvent(log *types.Log) (*PairImplUpgraded, error) {
	event := "Upgraded"
	if log.Topics[0] != pairImpl.abi.Events[event].ID {
		return nil, errors.New("event signature mismatch")
	}
	out := new(PairImplUpgraded)
	if len(log.Data) > 0 {
		if err := pairImpl.abi.UnpackIntoInterface(out, event, log.Data); err != nil {
			return nil, err
		}
	}
	var indexed abi.Arguments
	for _, arg := range pairImpl.abi.Events[event].Inputs {
		if arg.Indexed {
			indexed = append(indexed, arg)
		}
	}
	if err := abi.ParseTopics(out, indexed, log.Topics[1:]); err != nil {
		return nil, err
	}
	out.Raw = log
	return out, nil
}

// UnpackError attempts to decode the provided error data using user-defined
// error definitions.
func (pairImpl *PairImpl) UnpackError(raw []byte) (any, error) {
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["AddressEmptyCode"].ID.Bytes()[:4]) {
		return pairImpl.UnpackAddressEmptyCodeError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ERC1967InvalidImplementation"].ID.Bytes()[:4]) {
		return pairImpl.UnpackERC1967InvalidImplementationError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ERC1967NonPayable"].ID.Bytes()[:4]) {
		return pairImpl.UnpackERC1967NonPayableError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["EnforcedPause"].ID.Bytes()[:4]) {
		return pairImpl.UnpackEnforcedPauseError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ExpectedPause"].ID.Bytes()[:4]) {
		return pairImpl.UnpackExpectedPauseError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["FailedCall"].ID.Bytes()[:4]) {
		return pairImpl.UnpackFailedCallError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["InvalidInitialization"].ID.Bytes()[:4]) {
		return pairImpl.UnpackInvalidInitializationError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ListFailToFindPrev"].ID.Bytes()[:4]) {
		return pairImpl.UnpackListFailToFindPrevError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ListInvalidFindMaxCount"].ID.Bytes()[:4]) {
		return pairImpl.UnpackListInvalidFindMaxCountError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ListInvalidIndex"].ID.Bytes()[:4]) {
		return pairImpl.UnpackListInvalidIndexError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ListInvalidPrevNode"].ID.Bytes()[:4]) {
		return pairImpl.UnpackListInvalidPrevNodeError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["ListZeroData"].ID.Bytes()[:4]) {
		return pairImpl.UnpackListZeroDataError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["NotInitializing"].ID.Bytes()[:4]) {
		return pairImpl.UnpackNotInitializingError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["OwnableUnauthorizedAccount"].ID.Bytes()[:4]) {
		return pairImpl.UnpackOwnableUnauthorizedAccountError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairFillOrKill"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairFillOrKillError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInsufficientTradeVolume"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInsufficientTradeVolumeError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidAccountReserve"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidAccountReserveError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidAmount"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidAmountError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidInitializeData"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidInitializeDataError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidOrderId"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidOrderIdError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidOrderSide"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidOrderSideError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidPrevPrice"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidPrevPriceError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidPrice"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidPriceError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidReserve"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidReserveError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidRouter"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidRouterError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairInvalidTickSize"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairInvalidTickSizeError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairNotOwner"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairNotOwnerError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["PairUnknownPrices"].ID.Bytes()[:4]) {
		return pairImpl.UnpackPairUnknownPricesError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["SafeERC20FailedOperation"].ID.Bytes()[:4]) {
		return pairImpl.UnpackSafeERC20FailedOperationError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["UUPSUnauthorizedCallContext"].ID.Bytes()[:4]) {
		return pairImpl.UnpackUUPSUnauthorizedCallContextError(raw[4:])
	}
	if bytes.Equal(raw[:4], pairImpl.abi.Errors["UUPSUnsupportedProxiableUUID"].ID.Bytes()[:4]) {
		return pairImpl.UnpackUUPSUnsupportedProxiableUUIDError(raw[4:])
	}
	return nil, errors.New("Unknown error")
}

// PairImplAddressEmptyCode represents a AddressEmptyCode error raised by the PairImpl contract.
type PairImplAddressEmptyCode struct {
	Target common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error AddressEmptyCode(address target)
func PairImplAddressEmptyCodeErrorID() common.Hash {
	return common.HexToHash("0x9996b315c842ff135b8fc4a08ad5df1c344efbc03d2687aecc0678050d2aac89")
}

// UnpackAddressEmptyCodeError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error AddressEmptyCode(address target)
func (pairImpl *PairImpl) UnpackAddressEmptyCodeError(raw []byte) (*PairImplAddressEmptyCode, error) {
	out := new(PairImplAddressEmptyCode)
	if err := pairImpl.abi.UnpackIntoInterface(out, "AddressEmptyCode", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplERC1967InvalidImplementation represents a ERC1967InvalidImplementation error raised by the PairImpl contract.
type PairImplERC1967InvalidImplementation struct {
	Implementation common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ERC1967InvalidImplementation(address implementation)
func PairImplERC1967InvalidImplementationErrorID() common.Hash {
	return common.HexToHash("0x4c9c8ce3ceb3130f17f7cdba48d89b5b0129f266a8bac114e6e315a41879b617")
}

// UnpackERC1967InvalidImplementationError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ERC1967InvalidImplementation(address implementation)
func (pairImpl *PairImpl) UnpackERC1967InvalidImplementationError(raw []byte) (*PairImplERC1967InvalidImplementation, error) {
	out := new(PairImplERC1967InvalidImplementation)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ERC1967InvalidImplementation", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplERC1967NonPayable represents a ERC1967NonPayable error raised by the PairImpl contract.
type PairImplERC1967NonPayable struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ERC1967NonPayable()
func PairImplERC1967NonPayableErrorID() common.Hash {
	return common.HexToHash("0xb398979fa84f543c8e222f17890372c487baf85e062276c127fef521eea7224b")
}

// UnpackERC1967NonPayableError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ERC1967NonPayable()
func (pairImpl *PairImpl) UnpackERC1967NonPayableError(raw []byte) (*PairImplERC1967NonPayable, error) {
	out := new(PairImplERC1967NonPayable)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ERC1967NonPayable", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplEnforcedPause represents a EnforcedPause error raised by the PairImpl contract.
type PairImplEnforcedPause struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error EnforcedPause()
func PairImplEnforcedPauseErrorID() common.Hash {
	return common.HexToHash("0xd93c0665d6c96d04a8f174024fc4ddd66c250604aff22bbec808de86dd3637e3")
}

// UnpackEnforcedPauseError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error EnforcedPause()
func (pairImpl *PairImpl) UnpackEnforcedPauseError(raw []byte) (*PairImplEnforcedPause, error) {
	out := new(PairImplEnforcedPause)
	if err := pairImpl.abi.UnpackIntoInterface(out, "EnforcedPause", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplExpectedPause represents a ExpectedPause error raised by the PairImpl contract.
type PairImplExpectedPause struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ExpectedPause()
func PairImplExpectedPauseErrorID() common.Hash {
	return common.HexToHash("0x8dfc202bcfe9a735b559bee70674422512bc5c30f687046ae8778315fb81da44")
}

// UnpackExpectedPauseError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ExpectedPause()
func (pairImpl *PairImpl) UnpackExpectedPauseError(raw []byte) (*PairImplExpectedPause, error) {
	out := new(PairImplExpectedPause)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ExpectedPause", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplFailedCall represents a FailedCall error raised by the PairImpl contract.
type PairImplFailedCall struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error FailedCall()
func PairImplFailedCallErrorID() common.Hash {
	return common.HexToHash("0xd6bda27508c0fb6d8a39b4b122878dab26f731a7d4e4abe711dd3731899052a4")
}

// UnpackFailedCallError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error FailedCall()
func (pairImpl *PairImpl) UnpackFailedCallError(raw []byte) (*PairImplFailedCall, error) {
	out := new(PairImplFailedCall)
	if err := pairImpl.abi.UnpackIntoInterface(out, "FailedCall", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplInvalidInitialization represents a InvalidInitialization error raised by the PairImpl contract.
type PairImplInvalidInitialization struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error InvalidInitialization()
func PairImplInvalidInitializationErrorID() common.Hash {
	return common.HexToHash("0xf92ee8a957075833165f68c320933b1a1294aafc84ee6e0dd3fb178008f9aaf5")
}

// UnpackInvalidInitializationError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error InvalidInitialization()
func (pairImpl *PairImpl) UnpackInvalidInitializationError(raw []byte) (*PairImplInvalidInitialization, error) {
	out := new(PairImplInvalidInitialization)
	if err := pairImpl.abi.UnpackIntoInterface(out, "InvalidInitialization", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplListFailToFindPrev represents a ListFailToFindPrev error raised by the PairImpl contract.
type PairImplListFailToFindPrev struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ListFailToFindPrev()
func PairImplListFailToFindPrevErrorID() common.Hash {
	return common.HexToHash("0x80ce60d12beb14fd4646fb405d91b479b5676610cbcaeb9f106ca5a2a64de02b")
}

// UnpackListFailToFindPrevError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ListFailToFindPrev()
func (pairImpl *PairImpl) UnpackListFailToFindPrevError(raw []byte) (*PairImplListFailToFindPrev, error) {
	out := new(PairImplListFailToFindPrev)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ListFailToFindPrev", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplListInvalidFindMaxCount represents a ListInvalidFindMaxCount error raised by the PairImpl contract.
type PairImplListInvalidFindMaxCount struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ListInvalidFindMaxCount()
func PairImplListInvalidFindMaxCountErrorID() common.Hash {
	return common.HexToHash("0xab7e6393f2d6afe1003e64bc6c2663a42509e64b4b209b76878f30e22ca47ce3")
}

// UnpackListInvalidFindMaxCountError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ListInvalidFindMaxCount()
func (pairImpl *PairImpl) UnpackListInvalidFindMaxCountError(raw []byte) (*PairImplListInvalidFindMaxCount, error) {
	out := new(PairImplListInvalidFindMaxCount)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ListInvalidFindMaxCount", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplListInvalidIndex represents a ListInvalidIndex error raised by the PairImpl contract.
type PairImplListInvalidIndex struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ListInvalidIndex()
func PairImplListInvalidIndexErrorID() common.Hash {
	return common.HexToHash("0x39e60f736f23967f73fc20a9390601afcfe55c0f2b265682bf6fc0bdb04a8743")
}

// UnpackListInvalidIndexError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ListInvalidIndex()
func (pairImpl *PairImpl) UnpackListInvalidIndexError(raw []byte) (*PairImplListInvalidIndex, error) {
	out := new(PairImplListInvalidIndex)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ListInvalidIndex", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplListInvalidPrevNode represents a ListInvalidPrevNode error raised by the PairImpl contract.
type PairImplListInvalidPrevNode struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ListInvalidPrevNode()
func PairImplListInvalidPrevNodeErrorID() common.Hash {
	return common.HexToHash("0x182dfca5a0378357f4b17f3bbcec3e8140b027679c79b3f50dc39fc379f4d1e1")
}

// UnpackListInvalidPrevNodeError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ListInvalidPrevNode()
func (pairImpl *PairImpl) UnpackListInvalidPrevNodeError(raw []byte) (*PairImplListInvalidPrevNode, error) {
	out := new(PairImplListInvalidPrevNode)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ListInvalidPrevNode", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplListZeroData represents a ListZeroData error raised by the PairImpl contract.
type PairImplListZeroData struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error ListZeroData()
func PairImplListZeroDataErrorID() common.Hash {
	return common.HexToHash("0x0da93b97c1d0d4f37134df83396afa17866db0e6ee21918241aa4386fd2b17ed")
}

// UnpackListZeroDataError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error ListZeroData()
func (pairImpl *PairImpl) UnpackListZeroDataError(raw []byte) (*PairImplListZeroData, error) {
	out := new(PairImplListZeroData)
	if err := pairImpl.abi.UnpackIntoInterface(out, "ListZeroData", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplNotInitializing represents a NotInitializing error raised by the PairImpl contract.
type PairImplNotInitializing struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error NotInitializing()
func PairImplNotInitializingErrorID() common.Hash {
	return common.HexToHash("0xd7e6bcf8597daa127dc9f0048d2f08d5ef140a2cb659feabd700beff1f7a8302")
}

// UnpackNotInitializingError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error NotInitializing()
func (pairImpl *PairImpl) UnpackNotInitializingError(raw []byte) (*PairImplNotInitializing, error) {
	out := new(PairImplNotInitializing)
	if err := pairImpl.abi.UnpackIntoInterface(out, "NotInitializing", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplOwnableUnauthorizedAccount represents a OwnableUnauthorizedAccount error raised by the PairImpl contract.
type PairImplOwnableUnauthorizedAccount struct {
	Account common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error OwnableUnauthorizedAccount(address account)
func PairImplOwnableUnauthorizedAccountErrorID() common.Hash {
	return common.HexToHash("0x118cdaa7a341953d1887a2245fd6665d741c67c8c50581daa59e1d03373fa188")
}

// UnpackOwnableUnauthorizedAccountError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error OwnableUnauthorizedAccount(address account)
func (pairImpl *PairImpl) UnpackOwnableUnauthorizedAccountError(raw []byte) (*PairImplOwnableUnauthorizedAccount, error) {
	out := new(PairImplOwnableUnauthorizedAccount)
	if err := pairImpl.abi.UnpackIntoInterface(out, "OwnableUnauthorizedAccount", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairFillOrKill represents a PairFillOrKill error raised by the PairImpl contract.
type PairImplPairFillOrKill struct {
	Arg0 common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairFillOrKill(address arg0)
func PairImplPairFillOrKillErrorID() common.Hash {
	return common.HexToHash("0x1b3c35621d221739c2ede5b76359cd72f00fcdd0f3c1ca9a89363289630a0851")
}

// UnpackPairFillOrKillError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairFillOrKill(address arg0)
func (pairImpl *PairImpl) UnpackPairFillOrKillError(raw []byte) (*PairImplPairFillOrKill, error) {
	out := new(PairImplPairFillOrKill)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairFillOrKill", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInsufficientTradeVolume represents a PairInsufficientTradeVolume error raised by the PairImpl contract.
type PairImplPairInsufficientTradeVolume struct {
	Arg0 *big.Int
	Arg1 *big.Int
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInsufficientTradeVolume(uint256 arg0, uint256 arg1)
func PairImplPairInsufficientTradeVolumeErrorID() common.Hash {
	return common.HexToHash("0x0bc1e7dd1ab60d1086d586d50e0918947f0377d5a9b8afef32e9fe79eb6dde5a")
}

// UnpackPairInsufficientTradeVolumeError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInsufficientTradeVolume(uint256 arg0, uint256 arg1)
func (pairImpl *PairImpl) UnpackPairInsufficientTradeVolumeError(raw []byte) (*PairImplPairInsufficientTradeVolume, error) {
	out := new(PairImplPairInsufficientTradeVolume)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInsufficientTradeVolume", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidAccountReserve represents a PairInvalidAccountReserve error raised by the PairImpl contract.
type PairImplPairInvalidAccountReserve struct {
	Arg0 common.Address
	Arg1 common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidAccountReserve(address arg0, address arg1)
func PairImplPairInvalidAccountReserveErrorID() common.Hash {
	return common.HexToHash("0x340b5752b685586482660d95a493667ad736142408c34baa4733c08e6eab99de")
}

// UnpackPairInvalidAccountReserveError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidAccountReserve(address arg0, address arg1)
func (pairImpl *PairImpl) UnpackPairInvalidAccountReserveError(raw []byte) (*PairImplPairInvalidAccountReserve, error) {
	out := new(PairImplPairInvalidAccountReserve)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidAccountReserve", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidAmount represents a PairInvalidAmount error raised by the PairImpl contract.
type PairImplPairInvalidAmount struct {
	Arg0 *big.Int
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidAmount(uint256 arg0)
func PairImplPairInvalidAmountErrorID() common.Hash {
	return common.HexToHash("0xa334626eea0b2dc962797b0f685ee0523fe2bcd2a6594469a4af487494149e71")
}

// UnpackPairInvalidAmountError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidAmount(uint256 arg0)
func (pairImpl *PairImpl) UnpackPairInvalidAmountError(raw []byte) (*PairImplPairInvalidAmount, error) {
	out := new(PairImplPairInvalidAmount)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidAmount", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidInitializeData represents a PairInvalidInitializeData error raised by the PairImpl contract.
type PairImplPairInvalidInitializeData struct {
	Arg0 [32]byte
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidInitializeData(bytes32 arg0)
func PairImplPairInvalidInitializeDataErrorID() common.Hash {
	return common.HexToHash("0xc587916e7ac1a1e12844a45df47fafc4a370819dadc4f10ff6738fa5c82ba9c0")
}

// UnpackPairInvalidInitializeDataError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidInitializeData(bytes32 arg0)
func (pairImpl *PairImpl) UnpackPairInvalidInitializeDataError(raw []byte) (*PairImplPairInvalidInitializeData, error) {
	out := new(PairImplPairInvalidInitializeData)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidInitializeData", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidOrderId represents a PairInvalidOrderId error raised by the PairImpl contract.
type PairImplPairInvalidOrderId struct {
	Arg0 *big.Int
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidOrderId(uint256 arg0)
func PairImplPairInvalidOrderIdErrorID() common.Hash {
	return common.HexToHash("0xfcfdf90256b415252db5a8b94244d974b35e0a551d3e1ecba0fe6f3001e3c1b4")
}

// UnpackPairInvalidOrderIdError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidOrderId(uint256 arg0)
func (pairImpl *PairImpl) UnpackPairInvalidOrderIdError(raw []byte) (*PairImplPairInvalidOrderId, error) {
	out := new(PairImplPairInvalidOrderId)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidOrderId", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidOrderSide represents a PairInvalidOrderSide error raised by the PairImpl contract.
type PairImplPairInvalidOrderSide struct {
	Arg0 uint8
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidOrderSide(uint8 arg0)
func PairImplPairInvalidOrderSideErrorID() common.Hash {
	return common.HexToHash("0x7cbdd081f46a78d23475a3e2db093012dc79fb9151c0c4820ae74c0be90d4421")
}

// UnpackPairInvalidOrderSideError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidOrderSide(uint8 arg0)
func (pairImpl *PairImpl) UnpackPairInvalidOrderSideError(raw []byte) (*PairImplPairInvalidOrderSide, error) {
	out := new(PairImplPairInvalidOrderSide)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidOrderSide", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidPrevPrice represents a PairInvalidPrevPrice error raised by the PairImpl contract.
type PairImplPairInvalidPrevPrice struct {
	Arg0 uint8
	Arg1 *big.Int
	Arg2 *big.Int
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidPrevPrice(uint8 arg0, uint256 arg1, uint256 arg2)
func PairImplPairInvalidPrevPriceErrorID() common.Hash {
	return common.HexToHash("0xc18aa606f1198f14779dfe3d5ce2874196de2e80392a112897873ad82fbb6761")
}

// UnpackPairInvalidPrevPriceError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidPrevPrice(uint8 arg0, uint256 arg1, uint256 arg2)
func (pairImpl *PairImpl) UnpackPairInvalidPrevPriceError(raw []byte) (*PairImplPairInvalidPrevPrice, error) {
	out := new(PairImplPairInvalidPrevPrice)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidPrevPrice", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidPrice represents a PairInvalidPrice error raised by the PairImpl contract.
type PairImplPairInvalidPrice struct {
	Arg0 *big.Int
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidPrice(uint256 arg0)
func PairImplPairInvalidPriceErrorID() common.Hash {
	return common.HexToHash("0x4e1404fe7b61380d71736260b6435667e80a0e41433c4d05b095608b244e378b")
}

// UnpackPairInvalidPriceError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidPrice(uint256 arg0)
func (pairImpl *PairImpl) UnpackPairInvalidPriceError(raw []byte) (*PairImplPairInvalidPrice, error) {
	out := new(PairImplPairInvalidPrice)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidPrice", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidReserve represents a PairInvalidReserve error raised by the PairImpl contract.
type PairImplPairInvalidReserve struct {
	Arg0 common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidReserve(address arg0)
func PairImplPairInvalidReserveErrorID() common.Hash {
	return common.HexToHash("0x7bdf264d5d3b1a5335cbe93f78765316cc584df66eeab3761ccbf7a4c3a7952b")
}

// UnpackPairInvalidReserveError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidReserve(address arg0)
func (pairImpl *PairImpl) UnpackPairInvalidReserveError(raw []byte) (*PairImplPairInvalidReserve, error) {
	out := new(PairImplPairInvalidReserve)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidReserve", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidRouter represents a PairInvalidRouter error raised by the PairImpl contract.
type PairImplPairInvalidRouter struct {
	Arg0 common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidRouter(address arg0)
func PairImplPairInvalidRouterErrorID() common.Hash {
	return common.HexToHash("0x143db1914fecb66e53bb3dd6913d520671235e09f7508b8bcb2968d0d7c97647")
}

// UnpackPairInvalidRouterError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidRouter(address arg0)
func (pairImpl *PairImpl) UnpackPairInvalidRouterError(raw []byte) (*PairImplPairInvalidRouter, error) {
	out := new(PairImplPairInvalidRouter)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidRouter", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairInvalidTickSize represents a PairInvalidTickSize error raised by the PairImpl contract.
type PairImplPairInvalidTickSize struct {
	Arg0 *big.Int
	Arg1 *big.Int
	Arg2 *big.Int
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairInvalidTickSize(uint256 arg0, uint256 arg1, uint256 arg2)
func PairImplPairInvalidTickSizeErrorID() common.Hash {
	return common.HexToHash("0x57568135393b39fa2d694de0f538d7f2c2984c5ffaa77d9eaa39dc37ed67fff9")
}

// UnpackPairInvalidTickSizeError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairInvalidTickSize(uint256 arg0, uint256 arg1, uint256 arg2)
func (pairImpl *PairImpl) UnpackPairInvalidTickSizeError(raw []byte) (*PairImplPairInvalidTickSize, error) {
	out := new(PairImplPairInvalidTickSize)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairInvalidTickSize", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairNotOwner represents a PairNotOwner error raised by the PairImpl contract.
type PairImplPairNotOwner struct {
	Arg0 *big.Int
	Arg1 common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairNotOwner(uint256 arg0, address arg1)
func PairImplPairNotOwnerErrorID() common.Hash {
	return common.HexToHash("0x54de53f75b3f56e6b65d7d247ae11c6d3e2e5a91bcaeb2910f5d0430f3ad39ee")
}

// UnpackPairNotOwnerError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairNotOwner(uint256 arg0, address arg1)
func (pairImpl *PairImpl) UnpackPairNotOwnerError(raw []byte) (*PairImplPairNotOwner, error) {
	out := new(PairImplPairNotOwner)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairNotOwner", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplPairUnknownPrices represents a PairUnknownPrices error raised by the PairImpl contract.
type PairImplPairUnknownPrices struct {
	Arg0 uint8
	Arg1 *big.Int
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error PairUnknownPrices(uint8 arg0, uint256 arg1)
func PairImplPairUnknownPricesErrorID() common.Hash {
	return common.HexToHash("0x9f1cfdfe0955b417ab9aa8e3ebd89bb6bb8a52c9f206287a84854b59902e2683")
}

// UnpackPairUnknownPricesError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error PairUnknownPrices(uint8 arg0, uint256 arg1)
func (pairImpl *PairImpl) UnpackPairUnknownPricesError(raw []byte) (*PairImplPairUnknownPrices, error) {
	out := new(PairImplPairUnknownPrices)
	if err := pairImpl.abi.UnpackIntoInterface(out, "PairUnknownPrices", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplSafeERC20FailedOperation represents a SafeERC20FailedOperation error raised by the PairImpl contract.
type PairImplSafeERC20FailedOperation struct {
	Token common.Address
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error SafeERC20FailedOperation(address token)
func PairImplSafeERC20FailedOperationErrorID() common.Hash {
	return common.HexToHash("0x5274afe73c98b4749fc91ffae6b7b574e7842cb2144a159e9377a5f20b32edf9")
}

// UnpackSafeERC20FailedOperationError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error SafeERC20FailedOperation(address token)
func (pairImpl *PairImpl) UnpackSafeERC20FailedOperationError(raw []byte) (*PairImplSafeERC20FailedOperation, error) {
	out := new(PairImplSafeERC20FailedOperation)
	if err := pairImpl.abi.UnpackIntoInterface(out, "SafeERC20FailedOperation", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplUUPSUnauthorizedCallContext represents a UUPSUnauthorizedCallContext error raised by the PairImpl contract.
type PairImplUUPSUnauthorizedCallContext struct {
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error UUPSUnauthorizedCallContext()
func PairImplUUPSUnauthorizedCallContextErrorID() common.Hash {
	return common.HexToHash("0xe07c8dba242a06571ac65fe4bbe20522c9fb111cb33599b799ff8039c1ed18f4")
}

// UnpackUUPSUnauthorizedCallContextError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error UUPSUnauthorizedCallContext()
func (pairImpl *PairImpl) UnpackUUPSUnauthorizedCallContextError(raw []byte) (*PairImplUUPSUnauthorizedCallContext, error) {
	out := new(PairImplUUPSUnauthorizedCallContext)
	if err := pairImpl.abi.UnpackIntoInterface(out, "UUPSUnauthorizedCallContext", raw); err != nil {
		return nil, err
	}
	return out, nil
}

// PairImplUUPSUnsupportedProxiableUUID represents a UUPSUnsupportedProxiableUUID error raised by the PairImpl contract.
type PairImplUUPSUnsupportedProxiableUUID struct {
	Slot [32]byte
}

// ErrorID returns the hash of canonical representation of the error's signature.
//
// Solidity: error UUPSUnsupportedProxiableUUID(bytes32 slot)
func PairImplUUPSUnsupportedProxiableUUIDErrorID() common.Hash {
	return common.HexToHash("0xaa1d49a4c084bfa9aeeee2a0be65267a7f19ba7e1476b114dac513d2c14cb563")
}

// UnpackUUPSUnsupportedProxiableUUIDError is the Go binding used to decode the provided
// error data into the corresponding Go error struct.
//
// Solidity: error UUPSUnsupportedProxiableUUID(bytes32 slot)
func (pairImpl *PairImpl) UnpackUUPSUnsupportedProxiableUUIDError(raw []byte) (*PairImplUUPSUnsupportedProxiableUUID, error) {
	out := new(PairImplUUPSUnsupportedProxiableUUID)
	if err := pairImpl.abi.UnpackIntoInterface(out, "UUPSUnsupportedProxiableUUID", raw); err != nil {
		return nil, err
	}
	return out, nil
}
